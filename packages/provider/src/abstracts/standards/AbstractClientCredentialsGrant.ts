...
import { UseUserByClient } from '../../traits/UseUserByClient'
import { AbstractGrant } from '../AbstractGrant'

export abstract class AbstractClientCredentialsGrant extends Mixed(
  AbstractGrant,
  ...
  protected readonly generateRefreshTokenForClientCredentials: boolean = false
  ...
  protected async createToken(context: Context): Promise<IToken> {
    ...
    const [
      ...
      this.generateRefreshTokenForClientCredentials
  ...
  protected abstract override getUserByClient(
    client: IClient,
    context: Context,
